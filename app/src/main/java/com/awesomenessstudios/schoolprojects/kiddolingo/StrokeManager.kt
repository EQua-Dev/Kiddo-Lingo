...
import com.awesomenessstudios.schoolprojects.kiddolingo.RecognitionTask.RecognizedInk
  ...
  private fun commitResult() {
    recognitionTask!!.result()?.let {
      ...
      status = "Successful recognition: " + it.text
        textRetrieved = it.text
      Common.recognizedText = it.text.toString()
      Log.d(TAG, "commitResult: ${it.text.toString()}")
      Log.d(TAG, "commitResult: ${Common.recognizedText}")
      val mainActivity = Game()

      mainActivity.plateNumber = Common.recognizedText

      Log.d(TAG, "commitResultMA: ${mainActivity.plateNumber}")
  ...
  fun recognize(): Task<String?> {
    ...
    return modelManager
      ...
      .onSuccessTask { result: Boolean? ->
        ...
        val doThis = recognitionTask!!.run().toString()
        Log.d(TAG, "recognize: $doThis")
        recognitionTask!!.run()
