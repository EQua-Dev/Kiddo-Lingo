...
class ModelManager {
  ...
  fun setModel(languageTag: String): String {
    ...
    modelIdentifier = try {
      Log.d(TAG, "setModel: trying with download")
  ...
  fun checkIsModelDownloaded(): Task<Boolean?> {
    Log.d(TAG, "checkIsModelDownloaded: ${remoteModelManager.isModelDownloaded(model!!)}")
      ...
      .onSuccessTask(
        SuccessContinuation { remoteModels: Set<DigitalInkRecognitionModel>? ->
          ...
          for (model in remoteModels!!) {
            Log.i(TAG, "model$model: ${model.modelIdentifier.languageTag}")
